...
shinyServer(function(input, output, session) {
  ...
  tryCatch({
    ...
    # load layouts
    layout_names <- c(const_layout_default, detectorchecker::available_detectors,
                      const_layout_user)
      ...
      withProgress({
        ...
        if (input$level_radio == const_level_pixels) {
          ...
          if (input$dead_radio == const_density_plot) {
            ...
            output$dead_pixel_analysis_plot <- renderPlot({detectorchecker::plot_pixels_density(layout, adjust = 5., caption = FALSE)},
                                                          width = "auto", height = "auto")
          ...
          output$dead_pixel_module_analysis_plot <- renderPlot({
            withProgress({
              ...
              if (input$dead_radio == const_density_plot) {
                ...
                output$dead_pixel_module_analysis_plot <- renderPlot({detectorchecker::plot_pixels_density(layout, adjust = 0.5,
